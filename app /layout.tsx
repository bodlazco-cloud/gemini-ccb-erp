...
/**
 * CASTCRETE 360 - Root Layout
 * Enforces the 8-Department Structure and Global Status Monitoring.
 */
export default function AppLayout({ children }: { children: React.ReactNode }) {
  const [isSidebarOpen, setSidebarOpen] = useState(true);
  const [cashBufferAlert, setCashBufferAlert] = useState(false);

  // Departments as defined in the Mandates
  const departments = [
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900">
      ...
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sticky top-0 z-50">
        ...
        <div className="flex items-center gap-6">
          {/* Global Threshold Alert (Mandate: Finance/BOD) */}
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar Navigation */}
        <aside 
          className={`bg-white border-r border-slate-200 transition-all duration-300 ease-in-out z-40 ${
            isSidebarOpen ? 'w-72' : 'w-0 -translate-x-full'
          } lg:relative absolute inset-y-0`}
        >
          <div className="p-4 flex flex-col h-full">
            <nav className="flex-1 space-y-1">
              <div className="pb-4">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-3 mb-2">Main Dashboard</p>
                <a href="/" className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-blue-50 text-blue-700 font-semibold transition-all">
              ...
              {departments.map((dept) => (
                <a
                  key={dept.name}
                  href={dept.path}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-all group"
                >
                  <span className={`transition-colors group-hover:${dept.color}`}>
                    {dept.icon}
                  </span>
